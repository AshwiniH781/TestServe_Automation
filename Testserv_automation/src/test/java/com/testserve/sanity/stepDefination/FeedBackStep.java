...
public void click_on_submit_button() {
	createNode("Then", "the succssfull messgae is displayed");
	eventUtils.waitUntilElementIsPresent(driver, fee.Submit, 20);
	...
	w.until(ExpectedConditions.urlToBe("https://qa.testserv.ai/Feedback/Index"));
	String url=driver.getCurrentUrl();
	if(url.equals("https://qa.testserv.ai/Feedback/Index")) {
		logStatus("info", "users lands on index page succssfully");
	}
	else
	{
		logStatus("error", "users fails to lands on index page succssfully");
	}
}
